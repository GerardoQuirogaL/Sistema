...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(100, 10, utf8_decode("REPORTE DE INVITADOS"), 0, 1, 'C', 0);
        $this->Ln(7);
        ...
        $this->Cell(50, 10, utf8_decode('AREA DE ASISTENCIA'), 1, 0, 'C', 1);
...
$consulta_reporte_invitados = $conn->query("SELECT 
    nombre_apellido,
    area_asistencia,
    placas_vehiculo,
    modelo_marca,
...
while ($invitados = $consulta_reporte_invitados->fetch(PDO::FETCH_ASSOC)) {
    $i++;
    $pdf->Cell(10, 10, utf8_decode($i), 1, 0, 'C', 0);
    $pdf->Cell(60, 10, utf8_decode($invitados['nombre_apellido']), 1, 0, 'C', 0);
    $pdf->Cell(50, 10, utf8_decode($invitados['area_asistencia']), 1, 0, 'C', 0);
    $pdf->Cell(40, 10, utf8_decode($invitados['placas_vehiculo']), 1, 0, 'C', 0);
    $pdf->Cell(50, 10, utf8_decode($invitados['modelo_marca']), 1, 0, 'C', 0);
    $pdf->Cell(30, 10, utf8_decode($invitados['color_vehiculo']), 1, 0, 'C', 0);
    $pdf->Cell(30, 10, utf8_decode($invitados['estado']), 1, 1, 'C', 0);
}

$pdf->Output('Reporte_Invitado.pdf', 'I'); // I para mostrar en navegador, D para descargar
?>