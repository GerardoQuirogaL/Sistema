...
class CustomPDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(100, 10, utf8_decode("REPORTE DE ASISTENCIA POR PERSONA"), 0, 1, 'C', 0);
        $this->Ln(7);
        ...
        $this->Cell(80, 10, 'Numero Colaborador', 1, 0, 'C', 1);
...
// Validar entrada
if (empty($numero_colaborador) || empty($fecha_inicio) || empty($fecha_fin)) {
    die('Error: Todos los campos son obligatorios.');
}

// Consultar los datos
$sql = "SELECT id, numero_colaborador, fecha_entrada, fecha_salida 
        FROM asistencia_empleado 
        WHERE numero_colaborador = ? 
        AND fecha_entrada BETWEEN ? AND ?";
$stmt = $conn->prepare($sql);
$stmt->execute([$numero_colaborador, $fecha_inicio, $fecha_fin]);
...
if (empty($resultados)) {
    ...
    foreach ($resultados as $row) {
        ...
        $pdf->Cell(80, 10, $row['numero_colaborador'], 1, 0, 'C', 0);
        $pdf->Cell(80, 10, $row['fecha_entrada'], 1, 0, 'C', 0);
        $pdf->Cell(80, 10, $row['fecha_salida'], 1, 1, 'C', 0);
    }
}

$pdf->Output('Reporte_Asistencia_XPersona.pdf', 'I');
// Salida del PDF
$pdf->Output();
?>