...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(100, 10, utf8_decode("REPORTE DE ASISTENCIA PROVEEDORES"), 0, 1, 'C', 0);
        $this->Ln(7);
        ...
        $this->Cell(80, 10, utf8_decode('NOMBRE PROVEEDOR'), 1, 0, 'C', 1);
...
$consulta_reporte_asistenciaproveedor = $conn->query("SELECT 
    proveedor,
    fecha_entrada,
    fecha_salida
FROM asistencia_proveedor");

while ($asistencia = $consulta_reporte_asistenciaproveedor->fetch(PDO::FETCH_ASSOC)) {
    $i++;
    $pdf->Cell(40, 10, utf8_decode($i), 1, 0, 'C', 0);
    $pdf->Cell(80, 10, utf8_decode($asistencia['proveedor']), 1, 0, 'C', 0);
    $pdf->Cell(80, 10, utf8_decode($asistencia['fecha_entrada']), 1, 0, 'C', 0);
    $pdf->Cell(80, 10, utf8_decode($asistencia['fecha_salida']), 1, 1, 'C', 0);
}

$pdf->Output('Reporte_Asistencia_Proveedores.pdf', 'I'); // I para mostrar en navegador, D para descargar
?>
